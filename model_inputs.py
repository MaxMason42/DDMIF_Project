...
class ModelFeatures:
    ...
    def __init__(
        ...
        if split_tickers_individually:
            ...
            train = (
                trainvalid.reset_index().merge(train_dates, on="date").set_index("date").copy()
            )
            valid = (
                trainvalid.reset_index().merge(valid_dates, on="date").set_index("date").copy()
            )
        ...
        test_with_buffer = pd.concat(
            [
                pd.concat(
                    [
                        trainvalid[trainvalid.ticker == t].iloc[-(self.total_time_steps - 1):],
                        test[test.ticker == t]
                    ]
                ).sort_index()
                for t in tickers
        ...
        if lags:
            ...
            self.test_sliding = self._batch_data_smaller_output(test_with_buffer, True, self.lags)
        else:
    ...
    def get_column_definition(self):
        ...
        real_inputs = [tup for tup in column_definition if tup[1] == DataTypes.REAL_VALUED 
                       and tup[2] not in {InputTypes.ID, InputTypes.TIME, InputTypes.TARGET}]
        categorical_inputs = [tup for tup in column_definition if tup[1] == DataTypes.CATEGORICAL
                              and tup[2] not in {InputTypes.ID, InputTypes.TIME, InputTypes.TARGET}]
    ...
    def _batch_data(self, data, sliding_window):
        ...
        input_cols = [tup[0] for tup in self._column_definition if tup[2] not in {InputTypes.ID, InputTypes.TIME, InputTypes.TARGET}]
        ...
        if sliding_window:
            ...
            for _, sliced in data.groupby(id_col):
                ...
                sequence_lengths = [
                    (
                        lags if i != batch_size - 1 else lags - additional_time_steps_required
                    )
                ...
                if "active_entries" not in data_map:
                    ...
                    data_map["active_entries"].append(active_entries[sequence_lengths > 0, :, :])
